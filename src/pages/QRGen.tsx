...
export const QRGen = () => {
    ...
    return (
        <div className="flex flex-col items-center w-full px-5 mt-8 bg-[#f5f5f9]">
            ...
            <div className="flex justify-center w-full h-[calc(100svh-172px)] bg-cont-100 pt-10">
                <div className="flex flex-col w-1/3 px-20 me-80">
                    ...
                    <select value={selectedOption3} className="py-2 my-3 shadow-[0_0px_15px_-3px] rounded-full" name="date" id="date" onChange={handleChange3}>
                        <option value="1" disabled>Date</option>
                        <option value="2">12/12/2024</option>
                    </select>
                ...
                <div className="flex flex-col items-center relative -top-5">
                    ...
                    <button className={`flex justify-center w-[80%] rounded-full text-cont-100 ${((parseInt(selectedOption1) - 1) * (parseInt(selectedOption2) - 1) * (parseInt(selectedOption3) - 1)) > 0 ? "bg-[#03C088]" : "bg-[#BFC3C2]"} text-cont100 py-2`}>Download</button>
                    <p className="text-[#03C088]">{((parseInt(selectedOption1) - 1) * (parseInt(selectedOption2) - 1) * (parseInt(selectedOption3) - 1)) > 0 ? qty : ""}</p>
                </div>
            </div>
        </div>
    )
