...
export const Register = () => {
    ...
    function handleRegister() {
        ...
        auth.post("/api/v0/shops/register/", formData).then(response => {
    ...
    return (
        <>
            <form className="flex flex-col w-full relative mt-[46px]" method="post" onSubmit={(e) => { e.preventDefault(); return false }}>
                ...
                <input type={visiblityState} id="phone" className="mb-5 outline-none border-[#9e896a] border-[1px] rounded-full py-3 px-6" placeholder="Enter your phone number" />
                <div className="flex absolute right-5 bottom-[180px]" onClick={() => { visiblityState == "password" ? setVisiblityState("number") : setVisiblityState("password") }}>
                    <SvgShowPwd />
                </div>
                ...
                <input type="file" name="image" id="" accept="image/*" className="file:w-8 file:h-8 file:rounded-full file:border-none file:bg-black before:relative before:-right-[19px] before:-top-[2px] before:content-['+'] before:text-white text-white relative mt-5 -left-4 after:text-black after:absolute after:top-1 after:left-12 after:block after:h-3 after:content-['Upload-File']" />
